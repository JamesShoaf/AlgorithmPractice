...
fn helper(n: usize, k: usize, vec: &mut Vec<usize>) {
    if n > 0 {
        if k >= n + 25 {
            vec.push(26);
            helper(n - 1, k - 26, vec);
...
pub fn get_smallest_string(n: i32, k: i32) -> String {
    let abecedary: Vec<char> = String::from("abcdefghijklmnopqrstuvwxyz").chars().collect();
    let mut chars = Vec::new();
    helper(n as usize, k as usize, &mut chars);
    chars.reverse();
    chars
        .into_iter()
        .map(|int| abecedary[(int - 1) as usize])
        .collect()
}
