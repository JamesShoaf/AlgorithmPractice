...
const countSquares = (matrix) => {
  ...
  for (let row = 0; row < rows; row += 1) {
    const currentRowMap = Array(cols);
    ...
    for (let col = 0; col < cols; col += 1) {
      ...
      if (matrix[row][col] !== 1) currentRowMap[col] = 0;
        ...
        // check the map at [row - 1, col] and [row, col - 1] and set the value to
        // whichever is higher - 1
        let squares = Math.max(
          Math.max(currentRowMap[col - 1] ?? 0, previousRowMap[col] ?? 0) - 1, 1,
        );
