fn longest_palindrome_sequence(s: String) -> i32 {
    ...
    for (i, c) in (b'a'..=b'z').enumerate() { char_to_num.insert(c as char, i as i64); }
    for (i, c) in (b'A'..=b'Z').enumerate() { char_to_num.insert(c as char, (i + 26) as i64); }
    ...
    for (i, c) in s.enumerate() {
        ...
        longest = cmp::max(longest, i + 1 - *prev_mask_indices.entry(mask).or_insert(i + 1));
        for bit in 0..52 {
